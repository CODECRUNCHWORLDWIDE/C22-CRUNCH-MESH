...
stateDiagram-v2
  ...
  Candidate --> Candidate: split vote retries
```
*Raft's three-state leadership lifecycle, driven by election timeouts and term comparisons.*
...
sequenceDiagram
    ...
    C1->>LS: acquire lease token 33
    Note over C1: GC pause begins
    LS->>C2: lease expires grant token 34
    C2->>S: write with token 34
    S-->>C2: accepted
    Note over C1: wakes up still believes it holds lease
    C1->>S: write with token 33
    S-->>C1: rejected stale token
```
*Client1 wakes from a GC pause and writes with a stale token; storage fences it off.*
