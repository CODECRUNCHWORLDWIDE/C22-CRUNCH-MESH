...
```mermaid
flowchart TD
  A["App writes to socket for inventory ClusterIP"] --> B["eBPF hooks the connect call"]
  B --> C["Kernel picks a healthy backend pod"]
  C --> D{"Network policy allows"}
  D -- No --> E["Drop - visible in Hubble"]
  D -- Yes --> F["Kernel authenticates and encrypts"]
  F --> G{"L7 policy applies"}
  G -- No --> H["Delivered with no proxy touched"]
  G -- Yes --> I["Steered to per-node Envoy for L7"]
```
*A single cart-to-inventory packet's journey through the eBPF datapath.*
...
```mermaid
flowchart TD
  A{"Needs uniform mTLS or telemetry a library cannot verify"}
  A -- No --> B["No mesh: gateway plus library plus OTel"]
  A -- Yes --> C{"On Cilium CNI or willing to migrate"}
  C -- Yes and L4 dominant --> D["Cilium: kernel L4 and mTLS"]
  C -- No --> E{"Rich L7 needs and platform staff to run it"}
  E -- Yes --> F["Istio: consider ambient plus waypoints"]
  E -- No --> G["Linkerd: tiny proxy simplest ops"]
```
*The same decision flow as a diagram: mesh-at-all, then CNI, then L7-and-staffing.*
...
Next: the exercises put all three meshes on your cart topology and produce the numbers your ADR is built on. Continue to [the exercises](../exercises/README.md).
