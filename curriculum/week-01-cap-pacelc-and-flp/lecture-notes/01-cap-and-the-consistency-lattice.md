...
sequenceDiagram
  ...
  participant Primary
  participant Replica
  Client->>Primary: write new email
  Primary-->>Client: ack write ok
  Client->>Replica: read email confirm
  Note over Replica: replication lag window
  Replica-->>Client: old email returned
  Note over Client: user thinks change did not take
```
*The confirm read lands on a replica still inside the replication lag window, so it returns the pre-write email.*
...
```mermaid
flowchart TD
  N["N equals 3 replicas total"]
  W["Write quorum W equals 2"]
  R["Read quorum R equals 2"]
  N --> W
  N --> R
  W --> O["Overlap guaranteed since W plus R exceeds N"]
  R --> O
  O --> G["Every read sees at least one replica with the latest write"]
```
*With N=3, W=2, R=2 any write quorum and any read quorum are forced to share a replica.*
