...
```mermaid
flowchart TD
  F["FLP blocks guaranteed termination"]
  F --> E1["Escape 1 drop full asynchrony use timeouts"]
  F --> E2["Escape 2 drop determinism use randomization"]
  F --> E3["Escape 3 assume a failure detector"]
  E1 --> R["Raft and Paxos use this escape"]
  E2 --> B["Randomized and some BFT protocols"]
  E3 --> C["Chandra and Toueg failure detector theory"]
```
*FLP closes all three doors at once; every production system walks through exactly one.*
...
sequenceDiagram
  ...
  participant Peers
  participant Storage
  Note over OldLeader: long GC pause begins
  Peers->>Peers: timeout waiting on OldLeader
  Peers->>Peers: elect NewLeader with higher fencing token
  Note over OldLeader: GC pause ends leader still thinks it leads
  OldLeader->>Storage: write with old fencing token
  Storage-->>OldLeader: reject stale token
  Note over Storage: safety preserved despite the pause
```
*The resurrected leader still writes, but its stale fencing token gets it rejected by storage.*
