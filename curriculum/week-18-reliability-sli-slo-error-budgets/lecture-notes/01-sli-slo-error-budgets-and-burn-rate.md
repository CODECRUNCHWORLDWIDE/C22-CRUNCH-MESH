...
flowchart TD
  ...
  B -- "Yes" --> C["Ship risky deploys and drills"]
  C --> D["Spend budget deliberately"]
  D --> A
  B -- "No" --> E["Freeze feature launches"]
  E --> F["Work only on reliability"]
  F --> G["Budget recovers as window rolls forward"]
  G --> A
```
*The error-budget policy turns the ship-versus-stabilize decision into an automatic loop, not a meeting.*
...
```mermaid
flowchart TD
  A["Short window burn rate exceeds threshold"] --> C{"Both conditions true"}
  B["Long window burn rate exceeds threshold"] --> C
  C -- "Yes" --> D["Alert fires"]
  C -- "No" --> E["No alert"]
  D --> F["Fires fast and clears fast"]
```
*An alert only fires when the short window and long window agree the burn rate is too high.*
