...
```
*The breaker moves between closed, open, and half-open as the dependency fails and recovers.*
...
```mermaid
flowchart TD
  A["Cart service"] --> B["Payment pool 20 slots"]
  A --> C["Inventory pool 20 slots"]
  A --> D["Shipping pool 10 slots"]
  B --> E["Payment hangs and fills its pool"]
  E --> F["New payment calls rejected fast"]
  C --> G["Inventory keeps working"]
  D --> H["Shipping keeps working"]
```
*Separate bounded pools per dependency mean one hung dependency cannot starve calls to the others.*
