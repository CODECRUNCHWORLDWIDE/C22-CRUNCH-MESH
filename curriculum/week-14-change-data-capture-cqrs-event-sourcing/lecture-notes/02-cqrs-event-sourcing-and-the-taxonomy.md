...
```mermaid
flowchart LR
  W["Write model - orders table"] --> S["Change stream"]
  S --> R1["Elasticsearch search index"]
  S --> R2["Daily revenue rollup"]
  S --> R3["Per customer order history cache"]
  S --> R4["Iceberg table for the lakehouse"]
```
*One normalized write model feeds many purpose-built read model projections through the same change stream.*
...
```mermaid
flowchart TD
  A["Design uses events - which kind is it"] --> B{"Is the event log the source of truth"}
  B -->|Yes| B1["Event sourcing - confirm history is the asset and costs are accepted"]
  B -->|No| C{"Are read schemas separate and kept in sync by the change log"}
  C -->|Yes| C1["CQRS - confirm read models are rebuildable projections"]
  C -->|No| D{"Does the service publish events about its own current state db"}
  D -->|Via outbox or CDC| D1["Event driven correctly - confirm no dual write"]
  D -->|Via write then publish to Kafka| D2["Stop - that is a dual write - fix with outbox or CDC"]
```
*Classify a design before building it: event sourcing, CQRS, or event-driven, each with its own guardrail.*
...
Next: the exercises put this on the real `orders` stream — deploy Debezium, build an idempotent projector, and implement an event store with optimistic concurrency. Continue to [the exercises](../exercises/README.md).
