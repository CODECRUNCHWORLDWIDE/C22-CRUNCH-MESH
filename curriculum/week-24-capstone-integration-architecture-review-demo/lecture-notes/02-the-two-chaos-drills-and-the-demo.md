...
```mermaid
flowchart TD
  A["Primary region goes dark"] --> B["Cart - CRDT"]
  A --> C["Inventory - leased SKUs"]
  A --> D["Payment - Temporal workflow"]
  B --> E["Surviving region serves cart - merges on heal"]
  C --> F["Writes pause until lease expires then resume"]
  D --> G["Workflow resumes on surviving workers - charges once"]
```
*One region failure, three failure domains, three different degradations.*
...
sequenceDiagram
  ...
  Note over K: Leader broker dies
  K->>K: ISR elected new leader
  P->>K: Retry publish to new leader
  Note over C: Rebalance triggers re-pull
  C->>K: Re-pull in-flight batch
  C->>D: Check idempotency key
  D-->>C: Key already processed
  C->>C: Skip duplicate effect
```
*How a broker loss triggers redelivery, and how the dedup store turns it into a no-op.*
...
Next: the exercises walk you through tracing one order, running Drill A, and running Drill B. Continue to [the exercises](../exercises/README.md).
