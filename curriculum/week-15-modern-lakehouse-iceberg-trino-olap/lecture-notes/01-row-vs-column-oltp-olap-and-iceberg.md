...
```mermaid
flowchart TD
  A["Catalog - atomic pointer"] --> B["Table metadata file - schema partition spec snapshot list"]
  B --> C["Manifest list - one per snapshot"]
  C --> D["Manifest files - data file paths and stats"]
  D --> E["Data files - Parquet on object storage"]
```
*The Iceberg metadata tree, read top down from the catalog's atomic pointer to the underlying Parquet files.*
...
```mermaid
flowchart TD
  A["New query needs to run"] --> B{"Point read or single-row write"}
  B -->|Yes| C["Postgres OLTP - done"]
  B -->|No| D{"Small frequent aggregation an index or rollup fixes cheaply"}
  D -->|Yes| E["Postgres with index or materialized rollup"]
  D -->|No| F{"Large scan historical or would pollute OLTP cache"}
  F -->|Yes| G["Lakehouse Iceberg plus Trino"]
  F -->|No| H{"Needs a consistent historical view as of a date"}
  H -->|Yes| I["Lakehouse time travel"]
```
*Walking a new query down the OLTP versus lakehouse placement tree.*
