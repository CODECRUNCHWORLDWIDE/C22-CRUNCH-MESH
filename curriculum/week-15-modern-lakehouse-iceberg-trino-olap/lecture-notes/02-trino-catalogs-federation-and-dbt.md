...
```mermaid
flowchart TD
  A["Client SQL"] --> B["Coordinator - parse plan schedule"]
  B --> C["Worker"]
  B --> D["Worker"]
  B --> E["Worker"]
  C --> F["Iceberg connector"]
  D --> G["Postgres connector"]
  E --> H["Kafka connector"]
```
*The coordinator plans a query once and fans it out to workers, each reading through a connector to its own source.*
...
```mermaid
flowchart LR
  A["Land - raw CDC events in Iceberg"] --> B["Stage - dedupe and clean stg orders"]
  B --> C["Mart - build daily revenue rollup"]
  C --> D["Test - not null unique accepted values"]
  D --> E["Serve - dashboards and finance team"]
```
*The path from a raw change event to a trustworthy number on a dashboard.*
