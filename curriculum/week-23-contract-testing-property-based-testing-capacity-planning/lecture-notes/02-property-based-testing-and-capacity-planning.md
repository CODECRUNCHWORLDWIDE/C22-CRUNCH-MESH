...
```mermaid
flowchart LR
  A["Generator produces random input"] --> B["Run function under test"]
  B --> C{"Invariant holds"}
  C -->|Yes| A
  C -->|No| D["Shrinker simplifies failing input"]
  D --> E["Minimal counterexample"]
```
*The property-testing loop: generate, check the invariant, and shrink any failure down to a minimal counterexample.*
...
```mermaid
flowchart LR
  A["Example test"] --> B["Property test"]
  B --> C["Contract test"]
  C --> D["Capacity model"]
  D --> E["Integration test"]
  E --> F["Chaos drill"]
```
*Cheapest and earliest on the left, most expensive and latest on the right — push every bug as far left as it will go.*
