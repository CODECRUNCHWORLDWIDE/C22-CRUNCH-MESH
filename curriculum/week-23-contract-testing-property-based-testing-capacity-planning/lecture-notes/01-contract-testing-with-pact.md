...
```mermaid
sequenceDiagram
  participant C as Consumer order
  participant M as Pact mock provider
  participant F as Pact file
  participant B as Pact broker
  participant P as Real provider inventory
  C->>M: Declare expected interaction
  M-->>C: Recorded response
  C->>F: Generate pact file
  F->>B: Publish pact
  B->>P: Provider pulls pact
  P->>P: Verify against real service
  P->>B: Publish verification result
```
*The two phases of a Pact contract: the consumer records expectations against a mock, then the provider verifies them for real via the broker.*
...
```mermaid
flowchart TD
  A["can-i-deploy for a version"] --> B{"Every live consumer pact checked"}
  B -->|All pass| C["Allow deploy"]
  B -->|Any fail| D["Block deploy"]
  B -->|Any unknown never verified| D
```
*can-i-deploy is fail-closed: a passing verification is required, and an unverified pact blocks the deploy just like a failing one.*
