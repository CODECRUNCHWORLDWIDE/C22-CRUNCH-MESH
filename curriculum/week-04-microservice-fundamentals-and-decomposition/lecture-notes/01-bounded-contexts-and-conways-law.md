...
```mermaid
flowchart LR
  Catalog["catalog"] -->|"open host service"| Cart["cart"]
  Cart -->|"published language"| Order["order"]
  Inventory["inventory"] -->|"customer-supplier"| Order
  Order -->|"conformist"| Payment["payment"]
```
*Context map for the capstone domain — arrows point upstream to downstream, each labeled with its relationship pattern.*
...
A recurring confusion: learners hear "find the bounded contexts" and conclude "therefore deploy each as a microservice immediately." Lecture-1 §1.4 already separated the modeling decision from the deployment decision, but it deserves a concrete alternative: the **modular monolith.**
...
flowchart TD
  ...
  E --> F["Deployment"]
```
*The six-step decomposition procedure — each step guards against a specific failure if skipped.*
