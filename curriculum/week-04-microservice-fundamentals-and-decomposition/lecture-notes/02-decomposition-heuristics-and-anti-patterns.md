...
```mermaid
flowchart LR
  A["Entity service"] --> B["Chatty mesh"]
  B --> C["Shared database temptation"]
  C --> D["Distributed monolith"]
```
*The anti-pattern causal chain — cut the root entity service and the rest often resolve.*
...
```mermaid
flowchart LR
  N1["Need fresh read coupling ok"] --> P1["Synchronous gRPC"]
  N2["Need value frozen at a moment"] --> P2["Snapshot at write"]
  N3["Reads far exceed writes"] --> P3["Event fed read model"]
  N4["Cannot modify source"] --> P4["Change data capture"]
```
*Choosing a cross-service data pattern by the need it must satisfy.*
...
Next: the exercises put this on a real monolith. You'll draw a context map, score a proposed topology against the heuristics, and run a smell-detector over a topology spec. Continue to [the exercises](../exercises/README.md).
