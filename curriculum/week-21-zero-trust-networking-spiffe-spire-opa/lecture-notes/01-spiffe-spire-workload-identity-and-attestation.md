...
```mermaid
flowchart TD
  Server["SPIRE Server - trust domain CA"]
  Agent["SPIRE Agent - runs on every node"]
  Workload["Workload - example cart pod"]
  Server -- "issues and rotates SVIDs" --> Agent
  Agent -- "node attestation" --> Server
  Agent -- "workload attestation" --> Workload
  Workload -- "Workload API request" --> Agent
  Agent -- "hands back SVID" --> Workload
```
*The server signs identities; the per-node agent attests both itself and local workloads before handing out SVIDs.*
...
```mermaid
sequenceDiagram
  participant Platform as Cloud or Kubernetes platform
  participant Node as SPIRE agent on node
  participant Server as SPIRE server
  participant Workload as Workload process
  Platform->>Node: vouches for node identity
  Node->>Server: node attestation
  Server-->>Node: node trusted
  Workload->>Node: calls Workload API
  Node->>Node: inspects workload namespace service account uid
  Node-->>Workload: hands back short lived SVID
```
*Trust flows from the platform through the node-attested agent to the workload, with no secret handed out anywhere in the chain.*
