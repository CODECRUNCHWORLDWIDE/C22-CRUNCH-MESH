...
```mermaid
flowchart LR
  A["Pre-flight minutes 0 to 10"] --> B["Experiments minutes 10 to 75"]
  B --> C["Debrief minutes 75 to 85"]
  C --> D["Assign postmortems minutes 85 to 90"]
```
*The 90-minute gameday shape, from steady-state check to owned action items.*
...
sequenceDiagram
    ...
    P->>K: Publish order event
    K->>C: Deliver event
    Note over K: Broker dies then leader re-elects
    K->>C: Redeliver same event
    C->>I: Check idempotency key
    I-->>C: Already processed - no-op
    C->>C: Skip duplicate side effect
```
*Redelivery after broker failure hits the idempotency store, which turns the duplicate into a no-op.*
...
Next: the exercises put a real Chaos Mesh install, the six experiments, and the broker-loss EOS probe in your hands. Start with [Exercise 1 — Install and your first hypothesis-driven experiment](../exercises/exercise-01-install-and-first-experiment.md).
