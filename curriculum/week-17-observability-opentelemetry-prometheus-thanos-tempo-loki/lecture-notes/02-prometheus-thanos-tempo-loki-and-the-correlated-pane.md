...
```mermaid
flowchart LR
  A["Metric p99 spike"] --> B["Exemplar carries trace id"]
  B --> C["Tempo opens the trace"]
  C --> D["Logs for that span in Loki"]
```
*From a metric spike to the exact log line in three clicks, joined by one trace ID.*
...
```mermaid
flowchart TD
  A["Trace completes at gateway"] --> B{"Has an error span"}
  B -->|Yes| K["Keep trace"]
  B -->|No| C{"Duration over 500 ms"}
  C -->|Yes| K
  C -->|No| D["Sample at 1 percent"]
```
*Tail sampling keeps every interesting trace and thins out only the boring fast path.*
...
Next: the exercises put all of this on your cart topology — the Collector pipeline, a real Thanos deployment with PromQL, and the trace that survives Kafka. Continue to [the exercises](../exercises/README.md).
