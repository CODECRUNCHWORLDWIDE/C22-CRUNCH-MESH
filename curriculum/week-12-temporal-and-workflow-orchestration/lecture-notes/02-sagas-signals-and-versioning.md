...
flowchart LR
  ...
  B --> C["Ship Order"]
  C -->|failure| D["Compensate: Refund Charge"]
  D --> E["Compensate: Release Inventory"]
```
*Forward steps run left to right; on failure, compensations unwind in reverse order, LIFO.*
...
```mermaid
flowchart TD
  A["Workflow replay reaches GetVersion call"] --> B{"Version marker in history?"}
  B -->|No marker: old workflow| C["Return DefaultVersion, skip fraud check"]
  B -->|Marker equals 1: new workflow| D["Return version 1, run fraud check"]
  C --> E["Continue to charge step"]
  D --> E
```
*Old in-flight executions and newly started ones replay the same deployed code but take different branches, based on what their own history recorded.*
