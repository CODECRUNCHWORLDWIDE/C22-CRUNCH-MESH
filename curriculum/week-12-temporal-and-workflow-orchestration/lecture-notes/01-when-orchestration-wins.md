...
sequenceDiagram
  ...
  participant Worker
  Client->>Frontend: StartWorkflow
  Frontend->>History: persist WorkflowExecutionStarted
  History->>Matching: enqueue WorkflowTask
  Matching->>Worker: dispatch task
  Worker->>History: ScheduleActivity
  History->>Matching: enqueue ActivityTask
  Matching->>Worker: dispatch activity task
  Worker->>History: ActivityCompleted
  History->>Worker: resume workflow
```
*The server persists every decision; your worker is just an interchangeable executor that can be replaced at any point.*
...
```mermaid
flowchart TD
  A["Worker replays workflow from line 1"] --> B["Code reaches an ExecuteActivity call"]
  B --> C{"Is this activity already recorded in history?"}
  C -->|Yes| D["Return the recorded result immediately"]
  C -->|No| E["Schedule the activity for real"]
  D --> F["Continue to the next line"]
  E --> F
```
*Replay walks the same code path every time; recorded steps are fast-forwarded, only the first unrecorded step does real work.*
