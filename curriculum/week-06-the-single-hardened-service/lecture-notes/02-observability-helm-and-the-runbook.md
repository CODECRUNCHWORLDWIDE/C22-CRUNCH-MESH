...
sequenceDiagram
  ...
  Client->>Cart: AddItem request
  Cart->>Cart: start span, generate traceparent
  Cart->>Catalog: GetProduct with traceparent header
  Catalog->>Catalog: extract traceparent, start child span
  Catalog-->>Cart: response
  Cart-->>Client: response
```
*The traceparent header rides along on the gRPC call so catalog's span becomes a child of cart's span in one trace.*
...
```mermaid
flowchart LR
  A["Symptom"] --> B["Diagnosis"]
  B --> C["Mitigation"]
  C --> D["Verification"]
  D --> E["Escalation"]
```
*The five-stage shape every cart runbook playbook must follow, end to end.*
...
Next: the exercises. You'll audit `cart` against the twelve factors, implement correct graceful shutdown in Go, and write a complete hardened Kubernetes Deployment. Continue to [the exercises](../exercises/README.md).
