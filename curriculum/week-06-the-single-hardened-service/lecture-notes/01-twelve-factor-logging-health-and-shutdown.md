...
```mermaid
flowchart TD
  L["Liveness probe fails"] --> LA["Kubernetes restarts the container"]
  R["Readiness probe fails"] --> RA["Kubernetes removes pod from Service endpoints"]
  S["Startup probe not yet passed"] --> SA["Liveness and readiness probes held off"]
```
*Each of the three probes triggers a different Kubernetes action on failure.*
...
```mermaid
flowchart TD
  A["Pod marked Terminating"] --> B["preStop hook runs"]
  A --> C["Endpoints controller removes pod from Service"]
  B --> D["SIGTERM sent to container"]
  D --> E["Process drains in-flight work"]
  E --> F{"Exited before grace period ends"}
  F -->|Yes| G["Process exits cleanly"]
  F -->|No| H["SIGKILL - in-flight requests dropped"]
```
*The two parallel tracks of pod termination — endpoint removal and the preStop-then-SIGTERM drain — and why their race matters.*
