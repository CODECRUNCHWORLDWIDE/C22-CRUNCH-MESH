...
```mermaid
flowchart LR
  A["v2 at 10 percent"] --> B{"Healthy?"}
  B -->|"Yes"| C["v2 at 50 percent"]
  B -->|"No"| Z["Rollback to 0 percent"]
  C --> D{"Healthy?"}
  D -->|"Yes"| E["v2 at 100 percent"]
  D -->|"No"| Z
  E --> F["v1 scaled down"]
```
*Each canary stage is a cheap weight change, held only as long as the metric stays healthy.*
...
```mermaid
flowchart TD
  A["Meshed pod misbehaves"] --> B["Check proxy-status"]
  B -->|"Stale"| M["Mesh problem found"]
  B -->|"Synced"| C["Check describe pod"]
  C -->|"Policy missing"| M
  C -->|"Policy applied"| D["Check proxy-config"]
  D -->|"Route missing"| M
  D -->|"Route present"| E["Run analyze"]
  E -->|"Warning found"| M
  E -->|"Clean"| F["Check sidecar logs"]
  F -->|"RBAC or TLS error"| M
  F -->|"Upstream 500 only"| APP["Application problem"]
```
*Five ordered checks that isolate whether a misbehaving meshed pod is a mesh problem or an application problem.*
