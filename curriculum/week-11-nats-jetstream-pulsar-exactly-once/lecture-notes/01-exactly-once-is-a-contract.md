...
sequenceDiagram
  ...
  P->>C: Send message
  C-->>P: Send acknowledgement
  Note over P: Acknowledgement lost - was it the message or the ack
  P->>C: Retry send
  Note over C: Consumer may now see the message twice
```
*P cannot distinguish a lost message from a lost acknowledgement, so retrying trades loss for possible duplication.*
...
```mermaid
flowchart TD
  Producer --> Broker["Broker - stateless serving"]
  Consumer --> Broker
  Broker --> Bookie1["Bookie 1 - ledger storage"]
  Broker --> Bookie2["Bookie 2 - ledger storage"]
  Broker --> Bookie3["Bookie 3 - ledger storage"]
```
*Brokers handle routing and hold no data; bookies hold the ledgers, so either layer scales independently.*
