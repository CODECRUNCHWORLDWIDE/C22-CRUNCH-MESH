...
sequenceDiagram
  ...
  Note over A,B: Both hold milk tagged A1
  A->>A: Remove milk - observes tag A1
  B->>B: Add milk - creates new tag B1
  A->>B: merge adds and removes
  B->>A: merge adds and removes
  Note over A,B: Tag B1 is not in removes - milk present - add wins
```
*The OR-set add-wins trace: a concurrent re-add survives a remove because the remove only cancels the tags it actually observed.*
...
```mermaid
flowchart TD
  Start["What kind of state"] --> Counts["Counts"]
  Counts -->|"only grows"| GCounter["G-counter"]
  Counts -->|"grows and shrinks"| PNCounter["PN-counter"]
  Start --> Membership["Membership in a group"]
  Membership --> ORSet["OR-set"]
  Start --> Single["Single slot value"]
  Single -->|"only latest matters"| LWW["LWW-register"]
  Single -->|"conflicts must surface"| MV["MV-register"]
  Start --> Structured["Structured object"]
  Structured --> Map["CRDT map composing the above"]
```
*Which CRDT to reach for, from the shape of the state you need to converge.*
