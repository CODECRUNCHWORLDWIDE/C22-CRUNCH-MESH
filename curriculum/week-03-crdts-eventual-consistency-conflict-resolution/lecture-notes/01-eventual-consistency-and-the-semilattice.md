...
sequenceDiagram
  ...
  Note over A,B: State-based CvRDT
  A->>B: send whole state
  B->>B: merge state
  Note over A,B: Op-based CmRDT
  A->>B: broadcast operation
  B->>B: apply operation
```
*State-based ships the whole state and merges it; op-based ships each operation and applies it, demanding reliable causal delivery.*
...
```mermaid
flowchart TD
  A["Two replicas concurrently accept an update"] --> B{"Does merge-includes-both stay valid"}
  B -->|"Yes - accumulation is fine"| C["Use a CRDT"]
  B -->|"No - violates an invariant"| D["Use consensus or a single writer"]
```
*The one question that places any piece of state on the CRDT side or the coordination side of the boundary.*
