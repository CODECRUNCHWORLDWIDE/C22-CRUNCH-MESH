...
flowchart TD
  ...
  Quorum --> M["Metadata log"]
  M --> B1["Broker 1"]
  M --> B2["Broker 2"]
  M --> B3["Broker 3"]
  B1 --> Data["Serves partition data"]
  B2 --> Data
  B3 --> Data
```
*Controllers form a Raft quorum that owns the metadata log; brokers replay it and serve data independently.*
...
```mermaid
flowchart TD
  A["New topic - what do you keep"] --> B{"Need every historical record"}
  B -->|Yes| C["Event stream"]
  B -->|No - only latest per key| D["Changelog"]
  C --> E["cleanup policy delete - time or size retention"]
  D --> F["cleanup policy compact - keep latest per key"]
```
*Whether a topic is an event stream or a changelog decides its cleanup policy and what it costs to keep.*
...
Next: the exercises put all of this on a real cluster — Strimzi on Kind, a Go producer, a Python consumer group, and a head-to-head Kafka-vs-Redpanda benchmark. Continue to [the exercises](../exercises/README.md).
