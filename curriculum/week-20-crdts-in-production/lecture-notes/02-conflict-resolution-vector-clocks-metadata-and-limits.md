...
```mermaid
flowchart TD
  A["Compare two vector clocks"] --> B{"Does one dominate the other"}
  B -->|Yes| C["Causally ordered"]
  C --> D["Keep the later write"]
  B -->|No| E["Concurrent"]
  E --> F["Keep both as siblings"]
```
*Vector-clock comparison decides whether a write supersedes the other or both must be kept as siblings.*
...
```mermaid
flowchart LR
  A["Tombstones from OR-set removes"] --> B["Causal-stability GC"]
  C["Op history from op-based edits"] --> D["Snapshot and compaction"]
  E["Per-actor state growth"] --> F["Bounded fixed actor set"]
```
*Each source of CRDT metadata growth has a matching mitigation that keeps it bounded.*
...
Next: the exercises put an OR-set cart, an active-active Automerge cart, and vector-clock sibling resolution in your hands. Continue to [the exercises](../exercises/README.md).
