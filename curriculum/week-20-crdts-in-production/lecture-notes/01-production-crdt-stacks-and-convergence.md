...
```mermaid
flowchart TD
  A["Field or state to model"] --> B{"Document or primitive"}
  B -->|Document in-app state| C{"Interactive-speed editing"}
  C -->|Yes text or whiteboard| D["Yjs"]
  C -->|No richer object| E["Automerge"]
  B -->|Primitive data-tier infra| F["Redis Active-Active"]
```
*Choosing among Automerge, Yjs, and Redis Active-Active based on where the state lives and how it is used.*
...
sequenceDiagram
  ...
  A->>A: remove x tombstones old tag
  B->>B: add x with a new tag
  A->>B: merge state
  B->>A: merge state
  Note over A,B: new tag was never tombstoned so x is present
```
*An add-wins race: region B's concurrent add is not covered by region A's tombstone, so the item survives the merge.*
