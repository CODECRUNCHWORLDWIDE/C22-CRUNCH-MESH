...
```mermaid
flowchart LR
  A["Request"] --> B["Listener"]
  B --> C["Filter chain"]
  C --> D["Route table"]
  D --> E["Cluster"]
  E --> F["Endpoint"]
```
*The four nouns in order: a request crosses a listener, filter chain, route, and cluster before reaching an endpoint.*
...
sequenceDiagram
  ...
  CP->>EV: CDS new cluster
  EV-->>CP: ack
  CP->>EV: RDS route referencing new cluster
  EV-->>CP: ack
```
*ADS sequences the cluster before the route that references it, so Envoy is never briefly inconsistent.*
