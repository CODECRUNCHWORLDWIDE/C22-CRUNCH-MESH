...
```mermaid
flowchart LR
  A["Backend hiccups"] --> B["Requests fail"]
  B --> C["Clients retry"]
  C --> D["Load increases"]
  D --> A
```
*Without a budget, retries feed the same overload that triggered them, round and round.*
...
sequenceDiagram
  ...
  App->>BFF: GetCartScreen
  BFF->>Cart: GetCart
  Cart-->>BFF: cart items
  BFF->>Inv: GetStockBatch skus
  Inv-->>BFF: stock levels
  BFF-->>App: composed cart screen
```
*The BFF fans out to the gRPC backbone and returns one response shaped for the mobile screen.*
